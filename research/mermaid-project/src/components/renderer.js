...
import mermaid from 'mermaid'; // Import mermaid if not already imported
...
$(document).ready(function () {
  ...
  let oldAnswer = {}; // Initialize oldAnswer if not already defined
  let questionData = {}; // Initialize questionData if not already defined
  ...
  function questionSetup() {
    ...
    for (let i = 0; i < lis.length; i++) {
      let divs = lis[i].children; // Define divs within the loop
  ...
  function parseEntity(line) {
    ...
    let split_line = line.split('|'); // Define split_line
    let entity_name = split_line[0]; // Define entity_name
    ...
    if (split_line.length > 1) {
      let att = split_line[1].split(';'); // Define att
  ...
  function parseRelationship(line) {
    ...
    let card = line.substring(indexR + 1, indexL2); // Define card
    let cardinal = card.split('-'); // Define cardinal
  ...
  function enableQuestionEditing() {
    ...
    for (let i = 0; i < inQuestionMenuItems.length; i++) {
